...
export default auth((request) => {
	...
	if (isLoggedIn && isAdmin && isOnProfile) {
		console.log(
			'Admin user trying to access profile, redirecting to /dashboard',
		);
	...
	if (isLoggedIn && isAdmin && isOnAuth) {
		console.log(
			'Admin user trying to access auth pages, redirecting to /dashboard',
		);
		return Response.redirect(new URL('/dashboard', nextUrl));
	}
	...
	if (isLoggedIn && !isAdmin && isOnAuth) {
		console.log(
			'Non-admin user trying to access auth pages, redirecting to /profile',
		);
