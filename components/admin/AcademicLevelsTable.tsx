...
export function AcademicLevelsTable({ academicLevels }: AcademicLevelsTableProps) {
  const [levels, setLevels] = useState<AcademicLevel[]>(academicLevels);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingValues, setEditingValues] = useState<AcademicLevel | null>(null);

  const handleEdit = (level: AcademicLevel) => {
    setEditingId(level.id);
    setEditingValues({
      ...level,
      basePrice: typeof level.basePrice === 'string' 
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`/api/services/pricing/academic-levels/${editingId}`, {
        method: 'PATCH',
        headers: {
      ...
      setLevels(levels.map(level => 
        level.id === editingId ? editingValues : level
      ));
      ...
      toast.error(error instanceof Error ? error.message : 'Error updating academic level');
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/services/pricing/academic-levels/${id}`, {
        method: 'DELETE',
      });
      ...
      toast.error(error instanceof Error ? error.message : 'Error deleting academic level');
