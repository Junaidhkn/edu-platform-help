...
export async function GET(request: NextRequest) {
  try {
    ...
    const submissionList = await db.select()
      .from(submissions)
      .where(eq(submissions.status, status))
    ...
    if (freelancerIds.length > 0) {
      freelancerData = await db.select()
        .from(freelancers)
      ...
      if (freelancerIds.length > 1) {
        for (let i = 1; i < freelancerIds.length; i++) {
          const additionalFreelancers = await db.select()
            .from(freelancers)
    ...
    const submissionsWithRelations = submissionList.map(sub => {
      ...
      const freelancerInfo = freelancerData.find(f => f.id === sub.freelancerId) || null;
