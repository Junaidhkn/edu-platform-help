        ...
        try {
          ...
          if (userData?.email) {
            ...
            await transport.sendMail({
              ...
              text: `Good news! Your order #${orderData.id.slice(-6)} has been completed. You can download your files now.`,
              html: `
    ...
    if (freelancerData?.email) {
      try {
        ...
        const emailText = status === 'approved'
          ? `Congratulations! Your submission for order #${existingSubmission.orderId.slice(-6)} has been approved.`
          : `Your submission for order #${existingSubmission.orderId.slice(-6)} needs changes. Admin feedback: ${adminFeedback || 'No specific feedback provided.'}`;
        ...
        await transport.sendMail({
          ...
          text: emailText,
          html: `
            <h2>${status === 'approved' ? 'Submission Approved' : 'Submission Needs Changes'}</h2>
            <p>${emailText}</p>
            <p>Order: #${existingSubmission.orderId.slice(-6)}</p>
            ${status === 'rejected' ? `<p><strong>Feedback:</strong> ${adminFeedback || 'No specific feedback provided.'}</p>` : ''}
            <p>You can view the submission details <a href="${process.env.NEXTAUTH_URL}/freelancer/orders/${existingSubmission.orderId}">here</a>.</p>
          `
        });
      } catch (emailError) {
        console.error('Error sending freelancer notification email:', emailError);
        // Continue execution even if email fails
      }
    }
